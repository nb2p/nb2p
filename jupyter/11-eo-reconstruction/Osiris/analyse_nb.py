...
def analyse_nb(
    path, execute, verbose, match_pattern, self_reproduce, debug, analyse_all_dependency
):
    n_eo = utils.get_all_potential_execution_orders(path)
    if not n_eo:
        n_eo = []
    print(f"{path}: # possible execution orders={len(n_eo)}")
    print(f"{path}: possible execution orders[0]={n_eo[0]}")
