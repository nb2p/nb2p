...
from .utils import distinguish_local_modules, move_to_appropriate_location
...
class UserInterface:
    ...
    def analyse_executability(self):
        ...
        if (
            self.analyse_all_dependency is True
        ) and self._execute_strategy == "dependency":
            lst_executabilities = (
                self.analysizer.check_executability_on_all_potential_execution_paths(
                    self._verbose
                )
            )
    ...
    def analyse_reproducibility(self, match_pattern):
        ...
        if (
            ...
            lst_of_matched_ratios = (
                self.analysizer.check_reproducibility_on_all_potential_execution_paths(
                    self._verbose, match_pattern
                )
            )
            ...
            (
                ...
                match_ratio,
                match_cell_idx,
                source_code_from_unmatched_cells,
            ) = self.analysizer.check_reproducibility(
    ...
    def analyse_repeatablility(self):
        ...
        ) = self.analysizer.check_repeatablility(self._verbose, self._execute_strategy)
        return (
            num_of_reproducible_cells,
            num_of_cells,
            reproducible_ratio,
    ...
    def analyse_status_difference_for_a_cell(self, cell_index):
        if cell_index == None:
            raise ValueError(
                "cell_index argument should not be empty (None), please indicate the cell_index."
            )

        move_to_appropriate_location(self._nb_path)
        result = self.analysizer.check_status_difference_for_a_cell(
            self._execute_strategy, cell_index
        )

        if result is None:
            print(
                "Statements in this cell did not cause any status difference of self-defined variables"
            )
            return result
        elif result is -1:
            print(
                "Status difference of self-defined variables may occur before any execution of statements in this cell"
            )
            return result
        else:
            problematic_statement_index, supicious_statement = result
            print(
                "The potential statement for causing status difference is line",
                problematic_statement_index,
            )
            print("--------------> ", supicious_statement)
            print(
                "(Note that 0 indicates for the first line and empty lines are also included)"
            )
