...
def extract_source_code_from_unmatched_cells(cells, index_lst):
    ...
    unmatched_cells = [
        cell for (idx, cell) in enumerate(parsed_nb_cells) if idx in index_lst
    ]
...
def print_source_code_of_unmatched_cells(
    cells,
    analyse_strategy,
    index_lst,
    unmatched_original_outputs,
    unmtached_executed_outputs,
    ...
    if analyse_strategy == "normal":
        ...
        execution_count_lst = [cell.execution_count for cell in cells]
        OEC = sorted(
            range(len(execution_count_lst)), key=lambda k: execution_count_lst[k]
        )
        parsed_nb_cells = [cells[idx] for idx in OEC]
    ...
    for cell_idx, cell, original_output, executed_output in zip(
        index_lst,
        unmatched_cells,
        unmatched_original_outputs,
        unmtached_executed_outputs,
