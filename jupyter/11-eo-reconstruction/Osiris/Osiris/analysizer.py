...
from .ExecutePreprocessors import (
...
class Analysizer:
    def __init__(self, notebook_path, notebook_file):
        ...
        self._deep_copy_nb = copy.deepcopy(
            self._nb
        )  # store deepcopy of the given notebook to avoid unexpected manipulation
    ...
    def _clean_redundant_cells(self):
        ...
        parsed_nb_cells = [
            item
            for (idx, item) in enumerate(self._nb.cells)
            if idx not in invalid_cells_idx
        ]
    ...
    def _set_ep_check_repeatablility_mode(
        self, check_cell_idx, analyse_strategy, is_duplicate
    ):
        self._ep = SelfReproducibilityCheckPreprocessor(
    ...
    def check_reproducibility_on_all_potential_execution_paths(
        self, verbose, match_pattern
    ):
            ...
            if is_executable:
                try:
                    ...
                    if match_pattern == "strong":
                        ...
                        self._execute_nb()
                        original_outputs = extract_outputs_based_on_OEC_order(
                            self._nb.cells
                        )
                    ...
                    self._set_ep_as_dependency_mode(execution_order)
                    self._execute_nb()
                    executed_outputs = extract_outputs_based_on_normal_order(
                        self._nb.cells
                    )
                    ...
                    _ = extract_source_code_from_unmatched_cells(
                        self._nb.cells, unmatched_cell_idx
                    )
                    print(
                        "Reproducibility".ljust(40),
                        ":",
                        "number of matched cells: {num_of_matched_cells} ; number of cells: {num_of_cells}".format(
                            num_of_matched_cells=num_of_matched_cells,
                    ...
                    print(
                        "Reproducibility".ljust(40),
                        ":",
                        "matched ratio: {match_ratio} ; index of matched cells: {matched_cell_idx}".format(
                            match_ratio=round(match_ratio, 3),
                            matched_cell_idx=matched_cell_idx,
                        ),
                    )
    ...
    def check_repeatablility(self, verbose, analyse_strategy):
        ...
        for i in range(num_of_cells):
            ...
            is_duplicate = False
            self._nb = copy.deepcopy(self._deep_copy_nb)
            self._set_ep_check_repeatablility_mode(
                check_cell_idx, analyse_strategy, is_duplicate
            )
            if execution_order is not None:
                self._set_execution_order_for_ep_check_repeatablility_mode(
                    execution_order
                )
            self._execute_nb()
            ...
            if verbose:
                print(
                    "Check the {cell_idx} th cell among {num_of_cells} cells. Self-reproducibility result: {Self_reproduciblity_result}".format(
                        cell_idx=check_cell_idx,
                        num_of_cells=num_of_cells,
                        Self_reproduciblity_result=is_self_reproducible,
                    )
                )
        ...
        print(
            "Self reproducibility".ljust(40),
            ":",
            "number of repeatable cells: {num_of_self_reproducible_cells} ; number of cells: {num_of_cells}".format(
                num_of_self_reproducible_cells=num_of_self_reproducible_cells,
                num_of_cells=num_of_cells,
            ),
        )
        print(
            "Self reproducibility".ljust(40),
            ":",
            "self-reproduciblity ratio: {repeatablility_ratio} ; index of repeatable cells: {self_reproducible_cell_idx}".format(
                repeatablility_ratio=round(repeatablility_ratio, 3),
                self_reproducible_cell_idx=self_reproducible_cell_idx,
            ),
        )

        return (
            num_of_self_reproducible_cells,
            num_of_cells,
            repeatablility_ratio,
            self_reproducible_cell_idx,
        )
    ...
    def check_status_difference_for_a_cell(self, analyse_strategy, check_cell_idx):
        ...
        for i in range(num_of_statements):
            ...
            print(
                i,
                self._nb.cells[check_cell_idx].source.split("\n")[i],
                first_var_status,
                second_var_status,
            )
            if not (first_var_status == second_var_status):
