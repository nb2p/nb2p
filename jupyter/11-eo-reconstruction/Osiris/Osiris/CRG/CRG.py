...
class FilterTransformer(ast.NodeTransformer):
    ...
    def visit_AugAssign(self, node):
        ...
        tmp_node = ast.Assign(
            targets=[node.target],
            value=BinOp(left=target_tmp, op=node.op, right=node.value),
        )
        return tmp_node
...
class CRG:
    ...
    def update_symbol_table(self, node):
        ...
        func_records = [
            (tmp, "def") for tmp in class_ref_ids + func_ref_ids
        ] + func_records  # make sure defs are ahead of load
        vars_records = [
            (tmp, "store") for tmp in class_ref_ids + func_ref_ids
        ] + vars_records
        producer_set = set()
        consumer_set = set()
    ...
    def all_topo_util(self, all_paths, res, accum_producer_set, visited):
        ...
        for i in range(self.N):
            accum_producer_set_tmp = accum_producer_set.union(self.producer_list[i])
            if (
                self.consumer_list[i].issubset(accum_producer_set_tmp)
                and visited[i] == False
            ):
