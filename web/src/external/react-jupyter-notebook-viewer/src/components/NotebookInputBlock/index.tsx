...
import { type NotebookInputBlockType } from './types'
...
const themeDemo = EditorView.baseTheme({
  ...
  // '.cm-gutter': { backgroundColor: '#f8f8f8' }
...
const NotebookInputBlock: React.FC<NotebookInputBlockType> = (props) => {
  ...
  useEffect(() => {
    document.querySelectorAll('.markdown-block h1').forEach((h1) => {
      h1.classList.add(...['text-xl', 'font-extrabold', 'border-b', 'pt-3'])
      inputMarkdownDarkTheme
    ...
    document.querySelectorAll('.markdown-block h2').forEach((h2) => {
      h2.classList.add(...['text-lg', 'font-bold', 'border-b', 'pt-2'])
      inputMarkdownDarkTheme
    ...
    document.querySelectorAll('.markdown-block h3').forEach((h3) => {
      h3.classList.add(...['text-base', 'font-semibold', 'pt-1'])
    })

    document.querySelectorAll('.markdown-block h4').forEach((h4) => {
      h4.classList.add(...['text-sm', 'font-semibold', 'pt-1'])
    })

    document.querySelectorAll('.markdown-block h5').forEach((h5) => {
      h5.classList.add(...['text-xs', 'font-semibold', 'pt-1'])
    })

    document.querySelectorAll('.markdown-block h6').forEach((h6) => {
      h6.classList.add(...['text-xs', 'font-semibold', 'pt-1'])
    })
  ...
  const renderCodeBlock = () => (
    <div
      className={`input-${executionCount} flex w-full py-2 text-sm ${inputOuterClassName || ''} ${activeExecutionCount === executionCount
        ? `border-l-8 ${inputBorderClassName || 'border-blue-400'
        } my-2 pl-2 md:pl-0`
        : 'border-l-8 border-transparent my-2 pl-2 md:pl-0'
        }`}
    >
      <p
        className={`font-semibold min-w-16 ${inputTextClassName || ''
      ...
      <Card className='w-full overflow-x-auto rounded-none'>
        <CardContent className='p-0'>
          <div className={`w-full ${inputCodeBlockClassName || ''}`}>
            {/* <SynaxHighlighter
            ...
            <CodeMirror
              value={source && Array.isArray(source) ? source.join('') : source}
              theme={githubLight}
              basicSetup={{
  ...
  const renderMarkdownBlock = () => (
    <div
      className={`input-${executionCount} flex w-full py-2 ${inputOuterClassName || ''} ${activeExecutionCount === executionCount
        ? `border-l-8 ${inputBorderClassName || 'border-blue-400'
        } my-2 pl-2 md:pl-0`
        : 'border-l-8 border-transparent my-2 pl-2 md:pl-0'
        }`}
    >
      <div className={`font-semibold min-w-16 ${inputTextClassName || ''
          }`}
      />
      <div
        className={`markdown-block w-full ${inputMarkdownBlockClassName || ''
